...
async def test_valid_response_returns_bill_preview() -> None:
    llm_parser_cls = _import_llm_parser()
...
async def test_malformed_json_raises_parse_error() -> None:
    llm_parser_cls = _import_llm_parser()
    parse_error_cls = _import_parse_error()
    ...
    with patch("openai.AsyncOpenAI", return_value=mock_client):
        service = llm_parser_cls()
        with pytest.raises(parse_error_cls):
...
async def test_empty_content_raises_parse_error() -> None:
    ...
    mock_client.chat = MagicMock()
    mock_client.chat.completions = MagicMock()
    mock_client.chat.completions.create = AsyncMock(return_value=_make_mock_openai_response(""))
...
async def test_gas_bill_returns_correct_resource_type() -> None:
    ...
    assert result.resource_type.value == "GAS" or str(result.resource_type) in (
        "GAS",
        "ResourceType.GAS",
    )
    assert result.unit.value == "CUBIC_METER" or str(result.unit) in (
        "CUBIC_METER",
        "Unit.CUBIC_METER",
    )
