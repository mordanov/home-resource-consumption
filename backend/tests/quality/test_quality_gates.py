...
def test_no_hardcoded_openai_keys() -> None:
    ...
    assert not violations, (
        "Potential hardcoded OpenAI API keys found:\n" + "\n".join(violations)
    )
...
def test_no_hardcoded_jwt_secrets() -> None:
    ...
    for py_file in collect_python_files(APP_DIR):
        ...
        if 'JWT_SECRET_KEY = "' in content or "JWT_SECRET_KEY = '" in content:
            lines = content.splitlines()
            for i, line in enumerate(lines, start=1):
                if ('JWT_SECRET_KEY = "' in line or "JWT_SECRET_KEY = '" in line) and \
                   "env" not in line.lower() and "settings" not in line.lower():
                    violations.append(f"{py_file.relative_to(APP_DIR.parent)}:{i}: {line.strip()}")
    ...
    for py_file in collect_python_files(services_dir):
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                body_lines = [
                    line for line in func_lines[start:end]
...
@pytest.mark.skip(reason="Run manually: ruff check backend/app — skipped in unit test suite to avoid env dependency")
...
@pytest.mark.skip(reason="Run manually: mypy --strict backend/app — skipped in unit test suite to avoid env dependency")
...
def test_all_public_functions_have_type_annotations() -> None:
    ...
    assert not violations, (
        "Functions with missing type annotations (Selena gate):\n"
        + "\n".join(violations[:30])
    )
