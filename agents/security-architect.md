...
For this project, treat `documentation/speckit-prompt-resource-tracker.md` as the business source of truth. Security guidance must protect the authentication model, token lifecycle, data isolation, file upload boundary, LLM integration, and secrets management without replacing them with a different approach unless formally approved.

- Threat-model the **Resource Consumption Tracker** as a self-hosted web application with: JWT-based authentication, refresh token rotation, file upload (PDF/image), LLM integration (OpenAI API), ML prediction, PDF generation (WeasyPrint), and a single-user data model.
- **Authentication controls** (non-negotiable): bcrypt password hashing at cost ≥ 12; JWT access tokens (HS256, 15-minute TTL, payload `{ "sub": "<user_id>", "exp": ... }`); refresh tokens as 64-byte cryptographically random strings stored as SHA-256 hashes in the `refresh_tokens` table; refresh tokens rotated on every use (old token atomically revoked before new pair issued); refresh token delivered as `HttpOnly; Secure; SameSite=Strict` cookie — never in the JSON response body; `hashed_password` MUST NOT appear in any Pydantic response schema (Blocker if present).
- **Data isolation** (critical): every repository query touching `Bill` or `Prediction` MUST filter by `user_id`. A user must never be able to access another user's records. This is enforced server-side at the repository layer — client-supplied `user_id` from request bodies is not trusted.
- **File upload security**: validate MIME type (allowed: `application/pdf`, `image/jpeg`, `image/png`) and file size (max `MAX_UPLOAD_SIZE_MB` from env) before any processing. Reject without invoking PyMuPDF/pytesseract/OpenAI for disallowed types. File paths stored in `source_file_path` must not be user-controllable absolute paths that could enable path traversal.
- **LLM integration security**: the OpenAI API key must come from environment (`OPENAI_API_KEY` via pydantic-settings); never log, return, or embed it in error responses or tracebacks. LLM-returned data is validated by Pydantic before any persistence — do not pass raw LLM JSON to SQLAlchemy without schema validation.
- **Secrets management**: `JWT_SECRET_KEY` (≥ 256 bits), `OPENAI_API_KEY`, `DATABASE_URL` loaded from environment only. No secret may appear in source code, Git history, Docker image layers, CI log output, or error response bodies.
- **SQL injection**: all database queries MUST use SQLAlchemy parameterised statements. Raw SQL via `text()` is permitted for analytics aggregations only. String interpolation into SQL is prohibited.
- **CORS**: configured via `CORS_ORIGINS` env var; wildcard `*` is prohibited in any environment. Only the configured origin(s) may be allowed.
- **Error responses**: must follow RFC 7807 Problem Details and MUST NOT leak internal stack traces, database schema details, file system paths, or OpenAI API errors verbatim.
- **Define security tests for**: login with wrong password returns `401` and no token fields; expired access token returns `401`; refresh token replay after logout returns `401`; brute-force — 10 rapid wrong-password attempts return only `401`s (no 500s, no DB errors); `hashed_password` absent from all response schemas; file upload with disallowed MIME type returns `422`; bill/prediction query for user A never returns records owned by user B; `JWT_SECRET_KEY` shorter than 32 bytes fails startup validation.
