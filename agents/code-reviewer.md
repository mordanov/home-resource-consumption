...
For this project, treat `documentation/speckit-prompt-resource-tracker.md` as the business source of truth. Reviews must block changes that diverge from the documented architecture, stack, security model, data conventions, or API shape.

- Confirm the implementation targets the **Resource Consumption Tracker** — a single-user, self-hosted web application for household utility bill ingestion, LLM parsing, storage, and ML forecasting. Do not accept scope additions (SSO, multi-tenancy, CSV export, WebSockets, mobile apps, admin panels) without a formal amendment.
- **Backend checks**: Python 3.12 + FastAPI; SQLAlchemy async ORM 2.x; Alembic migrations for every schema change; Pydantic v2 for all request/response validation; no service class exceeding 30 lines in any method; `mypy --strict` must pass; `ruff` must pass with zero warnings; `structlog` for all logging (no `print()`).
- **Architecture checks**: strict four-layer separation (`api/v1/ → services/ → repositories/ → domain/`); no layer calling upward; `get_current_user` defined once in `app/api/deps.py`; all services injected via `Depends()`; `AuthService` must have zero imports from `BillService`, `ExportService`, or `AnalyticsService`.
- **Security blockers**: `hashed_password` absent from ALL Pydantic response schemas — any occurrence is a Blocker; passwords never logged; refresh token delivered as `HttpOnly; Secure; SameSite=Strict` cookie only (not in JSON body); `JWT_SECRET_KEY` loaded from environment, never hardcoded; CORS wildcard `*` is prohibited; all SQL uses SQLAlchemy parameterised statements (no string interpolation).
- **Data model checks**: all PKs are UUIDs; all tables include `created_at`/`updated_at` (`TIMESTAMPTZ`); monetary and consumption values stored as `NUMERIC(12, 4)` (not `FLOAT`); `bills` table uses soft deletion via `deleted_at` (no hard deletes); every DB schema change has a corresponding Alembic migration with a working `downgrade()`.
- **API checks**: all endpoints under `/api/v1/`; errors follow RFC 7807 Problem Details (`application/problem+json`); paginated list responses include `{ items, total, page, size, pages }`; every endpoint has OpenAPI `summary`, `description`, and at least one response example; PDF export uses `StreamingResponse` with `Content-Type: application/pdf`.
- **Data isolation check**: every `Bill` and `Prediction` repository query must filter by `user_id`; any query missing a `user_id` scope is a Blocker.
- **Frontend checks**: TypeScript `strict: true`; no `any` types; all routes except `/login` and `/register` wrapped in `<ProtectedRoute>`; access token stored in Zustand memory only (not `localStorage`, `sessionStorage`, or JS-accessible cookies); single Axios interceptor for token refresh; all React Query mutations have `onError` toast handlers; all data-displaying components render meaningful empty states; no direct `fetch()` calls.
- **Test checks**: no live OpenAI API calls in any test; overall coverage ≥ 85%; `app/services/` coverage ≥ 90%; regression tests present for every confirmed bug fix.
- **Stack discipline blockers**: no new runtime languages, databases, ORMs, or PDF libraries introduced without a constitution amendment; no headless browsers (no Playwright, Puppeteer, or Chrome).
- Require evidence of `docker compose up --build` success and `alembic upgrade head` passing against a fresh DB when deployment-facing files are changed.
